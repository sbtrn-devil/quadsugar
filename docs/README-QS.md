...
QS itself is ES 2019 compliant, and assumes the wrapped code is as well. Features that use syntax from more recent ES specs are not explicitly deprecated,
but they are not explicitly supported as well, so there can be subtle and cryptic errors if you try it - before trying any serious code, it makes sense to
